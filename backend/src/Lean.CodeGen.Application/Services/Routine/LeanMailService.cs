    ...
    public async Task<LeanApiResult> SendAsync(long id)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(mail.AttachmentIds))
            {
                ...
                var attachmentPaths = attachments.Select(x => Path.Combine(_httpContextAccessor.GetWebRootPath(), x.FilePath))
                  .Where(File.Exists)
                  .ToArray();
            ...
            foreach (var mail in mails)
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(mail.AttachmentIds))
                    {
                        var attachmentIds = mail.AttachmentIds.Split(',').Select(long.Parse).ToList();
                        var attachments = await _fileRepository.GetListAsync(x => attachmentIds.Contains(x.Id));
                        attachmentPaths = attachments.Select(x => Path.Combine(_httpContextAccessor.GetWebRootPath(), x.FilePath))
                          .Where(File.Exists)
                          .ToArray();
                    }
