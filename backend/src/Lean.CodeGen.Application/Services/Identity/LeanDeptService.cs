...
public class LeanDeptService : LeanBaseService, ILeanDeptService
{
  ...
  public async Task<LeanApiResult<long>> CreateAsync(LeanDeptCreateDto input)
  {
    ...
    {
      // 检查部门编码是否已存在
      var exists = await _deptRepository.AnyAsync(x => x.DeptCode == input.DeptCode);
      if (exists)
      {
        throw new LeanException("部门编码已存在");
      }

      // 检查部门名称是否已存在
      exists = await _deptRepository.AnyAsync(x => x.DeptName == input.DeptName);
      if (exists)
      {
        throw new LeanException("部门名称已存在");
      }
  ...
  public async Task<LeanApiResult> UpdateAsync(LeanDeptUpdateDto input)
  {
    return await ExecuteInTransactionAsync(async () =>
    {
      ...
      // 检查部门名称是否已存在
      var exists = await _deptRepository.AnyAsync(x => x.DeptName == input.DeptName && x.Id != input.Id);
      if (exists)
      {
        throw new LeanException("部门名称已存在");
      }
  ...
  {
    // 获取部门列表
    var deptResult = await GetPageAsync(input);
    if (!deptResult.Success)
    {
      throw new LeanException(deptResult.Message);
    }

    // 转换为导出DTO
    var exportDtos = deptResult.Data.Items.Select(x => new LeanDeptExportDto
    {
      DeptCode = x.DeptCode,
      DeptName = x.DeptName,
      ParentId = x.ParentId,
    ...
    try
    {
      ...
      foreach (var item in importResult.Data)
      {
        try
        {
          ...
          // 检查部门名称是否存在
          exists = await _deptRepository.AnyAsync(x => x.DeptName == item.DeptName);
          if (exists)
          {
            result.AddError(item.DeptCode, $"部门名称 {item.DeptName} 已存在");
            continue;
          }
  ...
  public async Task<List<LeanDeptTreeDto>> GetTreeAsync(LeanDeptQueryDto input)
  {
    ...
    // 构建树形结构
    return BuildDeptTree(deptResult.Data.Items);
  }
